...
COMMITTEE_JSON = DATA_DIR / "committee.json"
...
MAX_KEEP_COMMITTEE = 500  # committee.json 에 최대 몇 건(안건 단위)까지 보관할지
...
def build_committee_payloads(title, date, url, agenda_rows):
    """안건 행마다 구글시트/사이트에 공통으로 쓸 payload를 만든다.
    (실제 전송/저장은 호출부에서 처리)"""
...
def notify_committee(payloads):
    """안건 행들을 구글시트(Code.gs 웹앱)로 하나씩 전송한다. 실패해도 다음 행은 계속 시도."""
    if not COMMITTEE_WEBAPP_URL:
        return
    for payload in payloads:
        body = dict(payload)
        body["sheet"] = "committee"
        try:
            requests.post(
...
def main():
    ...
    committee_existing = load_json(COMMITTEE_JSON, [])
    committee_new = []
    ...
    for row in new_items:
        ...
        if "위원회" in row["title"]:
            try:
                ...
                if agenda_rows:
                    payloads = build_committee_payloads(
                        row["title"], row["date"], row["url"], agenda_rows
                    )
                    committee_new.extend(payloads)
                    if COMMITTEE_WEBAPP_URL:
                        notify_committee(payloads)
                        print(f"  [위원회] {row['title']} — 안건 {len(agenda_rows)}건 전송")
                    else:
                        print(
                            f"  [위원회] {row['title']} — 안건 {len(agenda_rows)}건 "
                            "(COMMITTEE_WEBAPP_URL 미설정이라 시트 전송은 건너뜀, committee.json에는 저장)"
                        )
                else:
    ...
    if committee_new:
        combined_committee = committee_new + committee_existing
        combined_committee = combined_committee[:MAX_KEEP_COMMITTEE]
        save_json(COMMITTEE_JSON, combined_committee)
        print(f"위원회 안건 저장 완료: 이번 {len(committee_new)}건 추가, 총 {len(combined_committee)}건")
